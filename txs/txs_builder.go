...
func shuffleWithNonceOrder(
	...
	for _, ntx := range ntxs[:total] {
		...
		if _, ok := byAddr[p]; !ok {
			byAddr[p] = []uint64{toAdd.Nonce.Counter, toAdd.Nonce.Counter}
		} else {
			byAddr[p][1] = toAdd.Nonce.Counter
		}
	...
	for addr, nonces := range byAddr {
		logger.With().Info("packed txs", addr, log.Uint64("from", nonces[0]), log.Uint64("to", nonces[1]))
	}
	return result
}
